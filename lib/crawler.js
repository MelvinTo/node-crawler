...
exports.Crawler = function(options) {
    ...
    self.options = _.extend({
        ...
        onDrain:        false,
        referer:        false,
        interval:       0, //how much time to wait after queue is processed, to avoid service DOS prevention from target website
    },options);
    ...
    var _release = function(opts) {
        console.log("_release is called");
        queuedCount--;
    ...
    var release = function(opts) {
        console.log("wait for " + self.options.interval + " seconds");
        setTimeout(_release(opts), self.options.interval);
    }
